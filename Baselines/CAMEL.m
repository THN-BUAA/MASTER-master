function [PD, PF, AUC, MCC, Balance, F1] = CAMEL(sources, target, LOC, alpha)
...
try
    [PD,PF,AUC,MCC, Balance, F1] = Performance(target(:,end),mean(probPosMatr,2)); %
catch
    PD=nan; PF=nan; AUC=nan; MCC=nan; Balance=nan; F1=nan;
end


Popt20 = CalculatePopt(target(:,end), double((mean(probPosMatr,2))>0.5), LOC);
